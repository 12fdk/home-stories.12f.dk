import type { Translation } from "../translation";

const fr: Translation = {
  "seo": {
    "title": "Home Stories - Suivi de rénovation gratuit pour iPhone",
    "description": "Suivez votre budget de rénovation, organisez les tâches et documentez l'avancement en photos. Exportez des rapports PDF professionnels. Gratuit sur iPhone - téléchargez maintenant !"
  },
  "nav": {
    "cta": "Télécharger l'app",
    "links": [
      "Fonctionnalités",
      "Comment ça marche",
      "Blog",
      "FAQ"
    ]
  },
  "ui": {
    "nav": {
      "openMenu": "Ouvrir le menu",
      "closeMenu": "Fermer le menu",
      "toggleTheme": "Basculer le mode sombre",
      "language": "Langue"
    },
    "header": {
      "eyebrow": "Suivi de rénovation pour iPhone",
      "committedSuffix": "engagé",
      "spent": "Dépensé",
      "committed": "Engagé",
      "left": "Restant"
    },
    "sectionLabels": {
      "features": "Fonctionnalités",
      "more": "Plus",
      "demo": "Démo",
      "reviews": "Avis",
      "howItWorks": "Comment ça marche",
      "writing": "Articles",
      "atAGlance": "En un coup d'œil"
    },
    "videoDemo": {
      "title": "L'app en action",
      "subtitle": "Trois écrans, filmés depuis l'app sur un iPhone. Sans maquettes, sans commentaire.",
      "tabs": [
        "Projets",
        "Budget",
        "Photos"
      ]
    },
    "blog": {
      "title": "Ce que nous avons appris sur les budgets",
      "subtitle": "Notes de rénovations qui ont dérapé, et de celles qui ont tenu.",
      "allPosts": "Tous les articles"
    },
    "footer": {
      "site": "Site",
      "contact": "Contact",
      "tagline": "Un suivi de rénovation pour iPhone. Conçu au Danemark par Robert Jensen."
    }
  },
  "header": {
    "headline": "Votre rénovation, chiffrée.",
    "subtitle": "Home Stories est un suivi de rénovation pour iPhone. Des budgets qui restent honnêtes, des tâches dans le bon ordre, des photos qui prouvent ce qui s'est passé — le tout exporté en PDF lisible par votre artisan.",
    "usersDescription": "Créé par un propriétaire en pleine rénovation, pour les propriétaires en pleine rénovation",
    "headlineMark": [
      2,
      3
    ]
  },
  "facts": [
    {
      "label": "Prix",
      "value": "Gratuit"
    },
    {
      "label": "Hors ligne",
      "value": "Totalement"
    },
    {
      "label": "Compte",
      "value": "Aucun"
    },
    {
      "label": "Synchro",
      "value": "iCloud"
    },
    {
      "label": "Requiert",
      "value": "iOS 17"
    }
  ],
  "features": {
    "title": "Quatre écrans font le travail",
    "subtitle": "Pas de tableur, pas de boîte à chaussures pleine de reçus, pas de pellicule impossible à fouiller.",
    "cards": [
      {
        "label": "Budget",
        "title": "Voyez le dépassement arriver",
        "subtitle": "Dépensé, engagé et restant sur un seul graphique. Vous découvrez le dépassement quand vous pouvez encore agir."
      },
      {
        "label": "Tâches",
        "title": "Gardez le bon ordre",
        "subtitle": "Regroupez le travail en tâches et articles par phase, puis cochez-les. L'électricien avant le plâtrier, à chaque fois."
      },
      {
        "label": "Photos",
        "title": "Prouvez ce qu'il y avait",
        "subtitle": "Des photos datées épinglées au projet forment une chronologie que vous pouvez parcourir — avant, pendant, et derrière le mur."
      },
      {
        "label": "Export",
        "title": "Remettez un PDF",
        "subtitle": "Budget, tâches, photos et notes dans un seul rapport que votre artisan, votre assureur ou un futur acheteur peut vraiment lire."
      }
    ]
  },
  "capabilities": {
    "title": "Et tout ce qui gravite autour",
    "subtitle": "Les quatre écrans font le gros du travail. Voici ce qui empêche le reste d'une rénovation de passer entre les mailles du filet.",
    "cards": [
      {
        "title": "Widgets et Live Activities",
        "subtitle": "Un anneau de budget sur votre écran verrouillé, les prochaines tâches en un coup d'œil, et un minuteur dans la Dynamic Island pendant que vous travaillez — sans ouvrir l'app."
      },
      {
        "title": "Suivi du temps",
        "subtitle": "Enregistrez les heures sur un projet et voyez où sont réellement passées les journées, détaillées visuellement à côté de l'argent."
      },
      {
        "title": "Notes et documents",
        "subtitle": "Gardez contrats, reçus et notes étiquetées avec photos, rattachés au projet concerné — pas dans un tiroir."
      },
      {
        "title": "Articles et listes d'achats",
        "subtitle": "Enregistrez vos achats avec prix et coordonnées du magasin, organisés par phase, pour que le coût réel s'affiche à côté de votre estimation."
      },
      {
        "title": "Recherche et extension de partage",
        "subtitle": "Cherchez dans tous vos projets instantanément, et enregistrez un produit directement depuis Safari, IKEA ou Amazon dans le bon projet."
      },
      {
        "title": "Collaboration en temps réel",
        "subtitle": "Partagez un projet via iCloud et gardez-le synchronisé avec votre conjoint, votre famille ou l'artisan sur le chantier."
      },
      {
        "title": "Priorités de projet",
        "subtitle": "Classez chaque projet en priorité Basse, Moyenne ou Haute et triez votre liste — par priorité, date ou nom — pour que le prochain chantier soit en tête."
      },
      {
        "title": "51 langues, accessible",
        "subtitle": "Entièrement traduite en 51 langues, avec prise en charge de VoiceOver et Dynamic Type partout. Fonctionne 100 % hors ligne."
      }
    ]
  },
  "howItWorks": {
    "title": "Cinq étapes, du début à la remise",
    "subtitle": "L'ordre que l'app attend, et l'ordre dans lequel une rénovation se déroule vraiment.",
    "steps": [
      {
        "title": "Fixez le budget",
        "subtitle": "Nommez le projet, mettez-y un montant et donnez-lui une échéance. C'est ce montant qui sert de référence à tout le reste."
      },
      {
        "title": "Listez le travail",
        "subtitle": "Découpez le chantier en tâches, puis ajoutez les matériaux, équipements et devis nécessaires à chacune. Des estimations maintenant, des reçus plus tard."
      },
      {
        "title": "Notez vos dépenses",
        "subtitle": "Saisissez les coûts au fur et à mesure. Le graphique répartit dépensé, engagé et restant, pour qu'un dépassement apparaisse tant que vous pouvez encore agir."
      },
      {
        "title": "Photographiez l'avancement",
        "subtitle": "Prenez vos photos depuis l'app. Elles sont datées et épinglées au projet, ce qui permet de trancher sur l'état du mur en mars."
      },
      {
        "title": "Exportez le rapport",
        "subtitle": "Un geste transforme budget, tâches, photos et notes en PDF. Envoyez-le à l'artisan, à l'assureur, ou au dossier que vous voudrez l'an prochain."
      }
    ]
  },
  "testimonials": {
    "title": "Ce que disent les propriétaires",
    "subtitle": "Par des gens en pleine rénovation",
    "comments": [
      "Home Stories a rendu la rénovation de notre cuisine tellement plus simple à gérer. Le suivi du budget montrait exactement où passait chaque couronne, et la chronologie photo est devenue un formidable témoignage de la transformation.",
      "Je rénove plusieurs pièces et Home Stories garde tout organisé. Chaque projet a ses propres tâches, son budget et ses photos. L'export PDF est parfait pour partager l'avancement avec les artisans.",
      "Les graphiques de budget sont incroyables. Je vois d'un coup d'œil ce qui est dépensé, ce qui reste et les coûts potentiels. Fini les tableurs - Home Stories réunit tout au même endroit."
    ]
  },
  "faq": {
    "title": "Vos questions, nos réponses",
    "qa": [
      {
        "question": "Home Stories est-elle gratuite ?",
        "answer": "Oui, c'est gratuit ! Nous proposons aussi une option Premium à vie, avec des fonctionnalités plus avancées pour les rénovateurs assidus."
      },
      {
        "question": "L'app fonctionne-t-elle hors ligne ?",
        "answer": "Absolument ! Home Stories est conçue pour fonctionner entièrement hors ligne, pour que vous puissiez accéder à vos projets, budgets et photos même sans connexion internet - idéal sur les chantiers."
      },
      {
        "question": "Puis-je partager des projets avec d'autres personnes ?",
        "answer": "Oui ! Grâce à la synchronisation iCloud, vos projets restent à jour sur tous vos appareils. Vous pouvez aussi exporter des rapports PDF professionnels à partager avec les artisans, la famille, ou à conserver dans vos archives."
      },
      {
        "question": "Comment exporter des rapports ?",
        "answer": "Ouvrez simplement votre projet, appuyez sur le bouton d'export et choisissez PDF. Home Stories génère un rapport professionnel avec le résumé de votre budget, l'avancement des tâches, les photos et les notes."
      },
      {
        "question": "Quels appareils sont pris en charge ?",
        "answer": "Home Stories est actuellement disponible sur iPhone et nécessite iOS 17.0 ou une version ultérieure. Nous nous concentrons d'abord sur la meilleure expérience possible sur iOS."
      },
      {
        "question": "Home Stories propose-t-elle des widgets ?",
        "answer": "Oui. Ajoutez un anneau de progression du budget et les tâches à venir sur votre écran d'accueil et votre écran verrouillé, et utilisez les Live Activities avec la Dynamic Island pour garder un minuteur de projet en vue pendant que vous travaillez — le tout sans ouvrir l'app."
      },
      {
        "question": "Puis-je collaborer avec un conjoint ou un artisan ?",
        "answer": "Oui. Partagez un projet via iCloud et il reste synchronisé en temps réel sur tous les appareils, pour qu'un conjoint, un proche ou un artisan puisse suivre le budget, les tâches et les photos à mesure qu'ils évoluent."
      },
      {
        "question": "Dans quelles langues Home Stories est-elle disponible ?",
        "answer": "Home Stories est entièrement traduite en 51 langues, dont l'anglais, l'allemand, le français, l'espagnol, l'italien, le danois, le néerlandais, le portugais, le japonais, le chinois, le coréen, et bien d'autres, avec une prise en charge complète de l'accessibilité VoiceOver et Dynamic Type."
      },
      {
        "question": "Puis-je suivre les coûts par catégorie, comme les matériaux et la main-d'œuvre ?",
        "answer": "Oui — les coûts vivent là où se trouve le travail. Découpez le projet en phases et en tâches, puis rattachez à chacune les matériaux, équipements, postes de main-d'œuvre et devis nécessaires. Le graphique de budget totalise le tout en dépensé, engagé et restant, et le rapport PDF le détaille."
      },
      {
        "question": "Comment gérer plusieurs artisans ?",
        "answer": "Regroupez le travail en phases et en tâches pour que chaque corps de métier sache quoi faire et quand — l'électricien avant le plaquiste. Partagez le projet via iCloud pour qu'un artisan suive le budget, les tâches et les photos en direct, ou exportez un PDF avec uniquement les sections utiles."
      },
      {
        "question": "Comment éviter de dépasser le budget ?",
        "answer": "Le graphique de budget montre le dépensé, l'engagé et le restant d'un coup d'œil : un dépassement apparaît pendant qu'il est encore temps d'agir. Ajoutez le widget sur l'écran d'accueil ou l'écran verrouillé pour garder l'anneau de budget en vue sans ouvrir l'app."
      },
      {
        "question": "Mes données de projet sont-elles sauvegardées ?",
        "answer": "Oui — avec la synchronisation iCloud activée, vos projets vivent dans votre compte iCloud et vous suivent sur un nouvel iPhone. Tout fonctionne aussi entièrement hors ligne ; les modifications se synchronisent dès que vous êtes de nouveau en ligne."
      }
    ]
  },
  "pricing": {
    "label": "Tarifs",
    "title": "Gratuit pour tout le chantier",
    "subtitle": "L'essentiel est gratuit, pour toujours. Un petit achat unique débloque les outils de budget avancés.",
    "plans": [
      { "name": "Gratuit", "period": "pour toujours", "features": ["Projets, tâches et phases illimités", "Chronologie photo avec photos datées", "Saisie des dépenses et prix des articles", "Fonctionne entièrement hors ligne", "Synchronisation iCloud entre appareils", "Widgets écran d'accueil et écran verrouillé"], "cta": "" },
      { "name": "Home Stories Pro", "period": "achat unique — sans abonnement", "features": ["Objectifs de budget et graphique budget/dépenses", "Analyse des coûts avancée", "Export PDF et CSV des rapports et relevés de temps", "Rappels de tâches et notifications d'échéance"], "cta": "Télécharger l'app — mise à niveau intégrée" }
    ],
    "footnote": "Prix unique sur l'App Store — il varie selon le pays."
  },
  "comparison": {
    "label": "Face au tableur",
    "title": "Pourquoi pas simplement un tableur ?",
    "subtitle": "Ça marche — jusqu'à la troisième semaine environ. La comparaison honnête.",
    "columns": { "them": "Un tableur", "us": "Home Stories" },
    "rows": [
      { "aspect": "Photos", "them": "Dans la pellicule ou un dossier, sans légende", "us": "Datées et épinglées à la chronologie du projet" },
      { "aspect": "Totaux", "them": "Des formules à écrire et entretenir soi-même", "us": "Dépensé, engagé et restant — automatique" },
      { "aspect": "Sur le chantier", "them": "Zoomer sur des cellules depuis son téléphone", "us": "Conçu pour iPhone, fonctionne entièrement hors ligne" },
      { "aspect": "Partage", "them": "Envoyer budget_v7_final_FINAL.xlsx par e-mail", "us": "Partage iCloud en direct, ou un PDF lisible par tous" },
      { "aspect": "Reçus", "them": "Une boîte à chaussures et de bonnes intentions", "us": "Photographiés et rangés avec le projet" },
      { "aspect": "Quand ça casse", "them": "Une formule supprimée échoue en silence", "us": "Rien à entretenir — la structure est intégrée" }
    ],
    "cta": "Pourquoi les tableurs finissent par lâcher"
  },
  "appBanner": {
    "title": "Chiffrez la prochaine.",
    "subtitle": "Gratuit sur l'App Store. Fonctionne hors ligne, sans compte, et se synchronise avec iCloud quand vous le souhaitez. Nécessite un iPhone avec iOS 17 ou une version ultérieure."
  }
};

export default fr;
